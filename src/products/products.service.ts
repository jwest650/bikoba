...
export class ProductsService {
  ...
  async create(
    ...
    if (dto.currency && dto.currency !== store.currency) {
      throw new BadRequestException(
        `Product currency must match the store's currency (${store.currency})`,
      );
    }
  ...
  async update(
    ...
    if (dto.currency && dto.currency !== existing.currency) {
      throw new BadRequestException(
        'Product currency cannot be changed — it inherits from the store',
      );
    }
